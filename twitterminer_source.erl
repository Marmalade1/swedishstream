...
-export([twitter_example/0, split_transformer/0, decorate_with_id/1, twitter_print_pipeline/2, twitter_producer/2, get_account_keys/1,buffer_pop_n/3]).
%-compile(export_all).
...
receive_tweets({init, URL, Keys}) ->
  ...
  SignedParams = oauth:sign("GET", URL, [{delimited, length}, {stall_warnings, true}, {language, "no"}], Consumer, AccessToken, AccessTokenSecret),
  %oauth:sign("GET", URL, [{delimited, length}, {stall_warnings, true}], Consumer, AccessToken, AccessTokenSecret),
